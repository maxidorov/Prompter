...
extension TextsViewController: UITableViewDelegate, UITableViewDataSource {
  ...
  func tableView(
    ...
    guard let cell = tableView.dequeueReusableCell(
      ...
      for: indexPath
    ) as? TextPreviewTableViewCell else {
      return UITableViewCell()
    }
    guard let textEntity =
            fetchedResultsController?.object(at: indexPath) else {
      return cell
    }
  ...
  func tableView(
    ...
    let deleteAction = UIContextualAction(
      ...
      AnalyticsTracker.shared.track(.deleteNote(symbolsCount: -1))
    ...
    let shareAction = UIContextualAction(
      ...
      guard let self = self,
            let textEntity
              = self.fetchedResultsController?.object(at: indexPath) else {
        return
      }
      self.presentActivityViewController(
        activityItems: [textEntity.title + textEntity.text]
      )
      completionHandler(true)
    }
    ...
    size = CGSize(
      width: shareIcon.size.width * sizeScale,
      height: shareIcon.size.height * sizeScale
    )
  ...
  fileprivate func deleteObject(at indexPath: IndexPath) {
    guard let managedObject =
            fetchedResultsController?.object(at: indexPath) else { return }
